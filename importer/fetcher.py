...
def fetch_account_snapshot(
    ...
    skip_globals: Optional[set] = None,
) -> AccountSnapshot:
    """
    ...
    Args:
        ...
        project_ids: If set, only fetch per-project data for these project IDs.
        skip_globals: Set of global resource names to skip fetching
            (e.g. {"service_tokens", "groups"}). "connections" and "repositories"
            cannot be skipped as they are referenced during project normalization.
    ...
    _skip = (skip_globals or set()) - {"connections", "repositories"}
    globals_fns = [
    ...
    with ThreadPoolExecutor(max_workers=threads) as ex:
        ...
        globals_model = Globals(
            connections=globals_results.get("connections", {}),
            repositories=globals_results.get("repositories", {}),
            service_tokens=globals_results.get("service_tokens", {}),
            groups=globals_results.get("groups", {}),
            notifications=globals_results.get("notifications", {}),
            webhooks=globals_results.get("webhooks", {}),
            privatelink_endpoints=globals_results.get("privatelink_endpoints", {}),
    ...
    if project_ids:
        project_items = [p for p in project_items if int(p.get("id") or 0) in project_ids]
