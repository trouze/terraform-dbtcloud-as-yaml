...
async def _run_generate(
    ...
    try:
        ...
        gitlab_deploy_token_kept = False
        terminal.info("")
        ...
        if gitlab_deploy_token_kept:
            tfvars_path = output_path / "gitlab_probe.auto.tfvars"
            tfvars_path.write_text(
                "# Auto-generated by GitLab deploy_token pre-flight probe\n"
                "enable_gitlab_deploy_token = true\n"
            )
            terminal.info(f"Wrote {tfvars_path.name} (enable_gitlab_deploy_token = true)")
...
async def _run_terraform_plan(
    ...
    try:
        ...
        if adopt_imports_exists:
            try:
                content = adopt_imports.read_text(encoding="utf-8")
                import_targets = [
                    m.group(1)
