"""Explore Source step page - browse source account entities, view reports, export data."""
...
def create_explore_source_page(
    state: AppState,
    on_step_change: Callable[[WorkflowStep], None],
    save_state: Callable[[], None],
) -> None:
    """Create the Explore Source step page with tabs."""
...
def _create_summary_tab(content: str, state: AppState) -> None:
    ...
    async def download_markdown():
        ...
        formatted = format_summary_as_markdown(
            content,
            state.fetch.account_name or "Unknown",
            state.source_account.account_id,
            state.source_account.host_url,
        )
        
        # Generate filename
        filename = generate_download_filename(
            "summary",
            state.fetch.account_name or "account",
        )
...
def _create_navigation(state: AppState, on_step_change: Callable[[WorkflowStep], None]) -> None:
    ...
    with ui.row().classes("w-full justify-between mt-4"):
        ui.button(
            f"Back to {state.get_step_label(WorkflowStep.FETCH_SOURCE)}",
            icon="arrow_back",
            on_click=lambda: on_step_change(WorkflowStep.FETCH_SOURCE),
        ).props("outline")
        
        ui.button(
            f"Continue to {state.get_step_label(WorkflowStep.SCOPE)}",
            icon="arrow_forward",
