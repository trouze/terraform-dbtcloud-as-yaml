# 31.03 Protection Management Intent Editing

> **Parent:** [31.02 Resource Protection](./31.02-Resource-Protection.md)
> **Standards:** [00.01 Standards of Development](./00.01-Standards-of-Development.md)
> **Contracts:** [Canonical Workflow Contracts](../docs/architecture/canonical-contracts.md)
...
- Grid/data model and actions: `importer/web/pages/utilities.py`
- Shared detail dialog and tabs: `importer/web/components/entity_table.py`
- Match page call pattern reference: `importer/web/pages/match.py`
- Match grid callback wiring reference: `importer/web/components/match_grid.py`
- Intent manager behavior (dense baseline persistence hooks): `importer/web/utils/protection_intent.py`
- Shared helpers (paths, env, subprocess): `importer/web/utils/terraform_helpers.py`
- Tests (existing — extend, don't duplicate):
  - `importer/web/tests/test_state_visibility_regression.py` — row merge and summary parity
  - `importer/web/tests/test_contract_enforcement.py` — no inline path/env violations
  - `importer/web/tests/test_terraform_helpers_equivalence.py` — shared helper fixtures
  - `importer/web/tests/test_adopt_protection.py`, `importer/web/tests/test_intent_key_consistency.py`
...
All changes to protection intent editing code and related UI **must** comply with the reuse-first and test-gate requirements in [00.01 Standards of Development § 0.17](./00.01-Standards-of-Development.md). Specifically:

1. Use canonical contracts (Contract 1, 2, 3) — no inline duplicates.
2. Pass the mandatory test gates before merge:
   - `test_contract_enforcement.py`
   - `test_state_visibility_regression.py`
   - `test_protection_grid_actions.py` (new)
   - `test_terraform_helpers_equivalence.py`
3. Encode any regressions found during implementation as new test cases + guardrails.

---

**Navigation:**
- [31.02 Resource Protection](./31.02-Resource-Protection.md) (parent)
- [43.03 Unified Protect & Adopt Pipeline](./43.03-Unified-Protect-Adopt-Pipeline.md)
- [00.01 Standards of Development](./00.01-Standards-of-Development.md)
- [Canonical Contracts](../docs/architecture/canonical-contracts.md)
