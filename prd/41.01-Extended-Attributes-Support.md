...
**Planning or adding another resource type?** Use the one-stop checklist: [41.02-Adding-New-Terraform-Object-Support.md](41.02-Adding-New-Terraform-Object-Support.md).
...
Per [41.02-Adding-New-Terraform-Object-Support.md](41.02-Adding-New-Terraform-Object-Support.md), add **Playwright E2E (UI) tests** for extended attributes:

- [ ] **Destroy page** (`test/e2e/test_destroy_protection.py`): Fixtures include `dbtcloud_extended_attributes` in state and (optionally) protection intent; assert Extended Attributes appear in resource list and type filter; unprotect flow and "Generate" moved-block flow work for at least one EXTATTR resource
- [ ] **Match page** (`test/e2e/test_match_protection.py` or equivalent): When YAML/state include extended attributes, Match grid shows extended attributes and protection checkbox/behavior can be asserted
...
**See [41.02-Adding-New-Terraform-Object-Support.md](41.02-Adding-New-Terraform-Object-Support.md) for the full checklist.** For extended attributes, fixtures must cover **all resource types** (PRJ, REP, PREP, ENV, JOB, EXTATTR) where protection or E2E workflows touch them.

Create test fixtures in `test/fixtures/` and `test/e2e/fixtures/` as needed:
...
- [ ] **Terraform state:** State JSON that includes `dbtcloud_extended_attributes` in both protected and unprotected module blocks (matching `modules/projects_v2/extended_attributes.tf`)
- [ ] **Protection intent:** JSON that includes EXTATTR keys (e.g. `project_key_ext_key`) so E2E and protection tests can assert on extended attributes
- [ ] **Report/source items:** Sample entities with `element_type_code: "EXTATTR"` and correct parent reference for hierarchy/cascade tests (e.g. in `importer/web/tests/test_protection_manager.py`)
