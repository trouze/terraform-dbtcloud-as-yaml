---
name: Jobs as Code Generator PRD
overview: "Create a detailed PRD for a \"Jobs as Code Generator\" feature integrated into the terraform-dbtcloud-yaml Web UI, supporting two workflows: (1) Adopt existing jobs under dbt-jobs-as-code management with linked_id, and (2) Clone/migrate jobs to another environment with optional Jinja templating."
todos:
  - id: prd-file
    content: Create PRD markdown file in tasks/ directory
    status: pending
  - id: user-stories-expand
    content: Expand user stories with detailed acceptance criteria
    status: pending
  - id: state-design
    content: Design detailed state management dataclasses
    status: pending
  - id: yaml-gen-logic
    content: Document YAML generation logic for both workflows
    status: pending
  - id: api-integration
    content: Document API integration with dbt Cloud client
    status: pending
  - id: ui-mockups
    content: Create detailed UI mockup descriptions
    status: pending
---
...
Leverage existing [dbt_jobs_as_code/client](dbt-jobs-as-code/dbt_jobs_as_code/client) for:
